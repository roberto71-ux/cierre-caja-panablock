...
# CxC — dos campos manuales
st.markdown("##### 📋 Cuentas por Cobrar")
st.caption("Ingresa los saldos de Cuentas por Cobrar (0 si no aplica):")
col_cxc1, col_cxc2 = st.columns(2)
with col_cxc1:
    st.number_input(
        "CxC al Mes Anterior ($)",
        min_value=0.0, value=0.0, step=0.01, format="%.2f",
        key="cxc_mes_anterior_manual",
    )
with col_cxc2:
    st.number_input(
        "CxC al Día de Ayer ($)",
    ...
    with st.spinner("Leyendo y procesando archivos…"):
        try:
            ...
            st.session_state.update({
                ...
                'cxc_mes_anterior': st.session_state.get('cxc_mes_anterior_manual', 0.0),
                'cxc_ayer':         st.session_state.get('cxc_ayer_manual', 0.0),
            })
    ...
    if generar:
        with st.spinner("Generando reportes…"):
            ...
            common = dict(
                ...
                cxc_mes_anterior=st.session_state.get('cxc_mes_anterior', 0.0),
                cxc_ayer=st.session_state.get('cxc_ayer', 0.0),
            )
            # Excel — siempre
            try:

...
def parse_recibos(path, log=print):
    ...
    cxc_cobros = 0.0   # suma de recibos con "cxc" en OBSERVACIONES
    ...
    for p in paths:
        ...
        # ── Detectar CXC en OBSERVACIONES ───────────────────────────────
        obs_m = re.search(r'OBSERVACIONES[:\s]*(.*)', text, re.IGNORECASE | re.DOTALL)
        if obs_m:
            obs_text = obs_m.group(1).strip()
            if re.search(r'\bcxc\b', obs_text, re.IGNORECASE):
                cxc_cobros = round(cxc_cobros + subtotal, 2)
                log(f"  Recibo CXC detectado (${subtotal:,.2f}) → CxC acumulado: ${cxc_cobros:,.2f}")
...
def generate_report(fecha_str, facturas, recibos, bg, gb,
    ...
    cxc_cobros_hoy = recibos.get('cxc_cobros', 0.0)
    saldo_cxc      = round(cxc_ayer - cxc_cobros_hoy, 2)

    spacer(22)
    spacer(23, 8)

    # ── Sección 2 — CUENTAS POR COBRAR (4 columnas: A, B, C, D) ────────
    section_hdr(24, 'CUENTAS POR COBRAR')
    # Encabezados: las 4 columnas con center alignment
    col_hdr(25, ['CxC al Mes Anterior', 'CxC al Día de Ayer',
                 'Cobros Hoy', 'Saldo CxC'])
    ws['A25'].alignment = CA   # col_hdr pone LA en A; forzamos CA aquí
    # Datos: los 4 valores numéricos con right alignment
    ws.row_dimensions[26].height = 20
    for col_l, val in zip(['A', 'B', 'C', 'D'],
                          [cxc_mes_anterior, cxc_ayer,
                           cxc_cobros_hoy, saldo_cxc]):
        c = ws[f'{col_l}26']
        c.value         = val
        c.font          = Font(name='Garamond', bold=True, size=11)
        c.fill          = mk_fill(LGRAY)
        c.alignment     = RA
        c.border        = BRD
        c.number_format = CURRENCY
    spacer(27, 8)

    # ── Sección 3 — PAGOS Y GASTOS ─────────────────────────────────
    section_hdr(28, 'PAGOS Y GASTOS')
    col_hdr(29, ['Descripción', 'Banco General', 'Global Bank', 'Total'])
    ...
    # ── Sección 4 — MOVIMIENTOS DE BANCOS ──────────────────────────
    ...
    ws['A41'].font      = Font(name='Garamond', size=11)
    ws['A41'].alignment = LA
    ...
    ws['A43'].font      = Font(name='Garamond', italic=True, size=9, color='595959')
    ws['A43'].alignment = LA
...
def generate_pdf_report(fecha_str, facturas, recibos, bg, gb,
    ...
    # ══════════════════════════════════════════════════════════════
    # SECCIÓN 2 — CUENTAS POR COBRAR  (4 columnas iguales)
    # Row 0: encabezado de sección
    # Row 1: encabezados de columna
    # Row 2: datos CxC                LGRAY bold
    # ══════════════════════════════════════════════════════════════
    cw_cxc = [CW / 4, CW / 4, CW / 4, CW / 4]
    data_cxc = [
        ['CUENTAS POR COBRAR', '', '', ''],
        ['CxC al Mes Anterior', 'CxC al Día de Ayer', 'Cobros Hoy', 'Saldo CxC'],
        [money_show(cxc_mes_anterior), money_show(cxc_ayer),
         money_show(cxc_cobros_hoy), money_show(saldo_cxc)],
    ]

    style_cxc = TableStyle(
        BASE
        + sec_hdr(4)
        + col_hdr(1, 4)
        + num_right(0, 3, 2, -1)
    ...
    t_cxc = Table(data_cxc, colWidths=cw_cxc)
    t_cxc.setStyle(style_cxc)
    story.append(t_cxc)
    story.append(Spacer(1, 0.08 * inch))

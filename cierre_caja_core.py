...
def generate_report(fecha_str, facturas, recibos, bg, gb,
    ...
    saldo_cxc      = round(cxc_mes_anterior - cxc_cobros_hoy, 2)

    spacer(22)
    spacer(23, 8)

    # ── Sección 2 — CUENTAS POR COBRAR (3 columnas: A:B fusionado, C, D) ──
    section_hdr(24, 'CUENTAS POR COBRAR')
    # Encabezados: A:B fusionado = 1ª columna, C = 2ª, D = 3ª
    ws.merge_cells('A25:B25')
    for ref, hdr in [('A25', 'CxC al Mes Anterior'),
                     ('C25', 'Cobros Hoy'),
                     ('D25', 'Saldo CxC')]:
        c = ws[ref]
        c.value     = hdr
    ...
    # Datos: A:B fusionado = cxc_mes_anterior, C = cobros hoy, D = saldo
    ws.merge_cells('A26:B26')
    for ref, val in [('A26', cxc_mes_anterior),
                     ('C26', cxc_cobros_hoy),
                     ('D26', saldo_cxc)]:
        c = ws[ref]
        c.value         = val
...
def generate_pdf_report(fecha_str, facturas, recibos, bg, gb,
    ...
    # ══════════════════════════════════════════════════════════════
    # SECCIÓN 2 — CUENTAS POR COBRAR  (3 columnas iguales)
    # Row 0: encabezado de sección
    # Row 1: encabezados de columna
    # Row 2: datos CxC                LGRAY bold
    # ══════════════════════════════════════════════════════════════
    cw_cxc = [CW / 3, CW / 3, CW / 3]
    data_cxc = [
        ['CUENTAS POR COBRAR', '', ''],
        ['CxC al Mes Anterior', 'Cobros Hoy', 'Saldo CxC Mes Anterior'],
        [money_show(cxc_mes_anterior), money_show(cxc_cobros_hoy), money_show(saldo_cxc)],
    ]

...
def generate_report(fecha_str, facturas, recibos, bg, gb,
    ...
    # Fila 9: encabezado de la sub-sección (justo bajo Cobros a Clientes)
    ws.row_dimensions[9].height = 18
    ws.merge_cells('A9:D9')
    c9 = ws['A9']
    c9.value     = 'Desglose de Cobros por Forma de Pago'
    c9.font      = Font(name='Arial', bold=True, size=10, color=DARK_BLUE)
    c9.fill      = mk_fill('D9E1F2')
    c9.alignment = LA
    c9.border    = BRD

    # Filas 10-14: una fila por cada forma de pago
    # Efectivo → fila 10 (usada por "Total a Depositar" más abajo)
    ...
    # Fila 15: Total a Depositar = Efectivo (D10)
    row_s1(15, 'Total a Depositar en el Día',
           d='=D10', bold=True, bg_color=MGRAY)
    row_s1(16, 'Ventas al Crédito',
           b=facturas['credito_subtotal'],
           c=facturas['credito_itbms'],
           d='=B16+C16')

    row_s1(17, 'Total Ventas del día',
           b='=B7+B16', c='=C7+C16', d='=D7+D16',
           bold=True, bg_color=LGRAY)

    spacer(18)   # separador entre sección 1 y sección 2

    # ── Sección 2 — PAGOS Y GASTOS ─────────────────────────────────
    section_hdr(19, 'PAGOS Y GASTOS')
    col_hdr(20, ['Descripción', '', '', 'Total'])

    gastos     = _merge_gastos(bg['gastos'], gb['gastos'])
    total_gast = round(sum(gastos.values()), 2)
    row_s2(21, 'Pago a Proveedores',
           d=(total_gast if total_gast else None), bold=True, bg_color=LGRAY)

    spacer(22)

    # ── Sección 3 — MOVIMIENTOS DE BANCOS ──────────────────────────
    section_hdr(23, 'MOVIMIENTOS DE BANCOS')
    col_hdr(24, ['Descripción', 'Banco General', 'Global Bank', ''])

    row_s3(25, 'Saldo Anterior',
           b=bg['saldo_anterior'], c=gb['saldo_anterior'])
    row_s3(26, 'Depósitos según extracto bancario (efectivo, ACH, TC, Yappy)',
    ...
    row_s3(27, 'Egresos según extracto bancario',
           b=bg_egresos if bg_egresos else None,
           c=gb_egresos if gb_egresos else None)

    row_s3(28, 'Saldo Bancario al Final del Día',
           b='=B25+B26-B27', c='=C25+C26-C27',
           bold=True, bg_color=LGRAY)

    row_s3(29, 'Cheques en Circulación',
           b=cheques_bg if cheques_bg else None,
           c=cheques_gb if cheques_gb else None)

    row_s3(30, 'Saldo Bancario Real Conciliado',
           b='=B28-B29', c='=C28-C29',
           bold=True, bg_color=MGRAY)
    ws['D29'].border = NO_BORDER
    ...
    ws['A32'].value     = 'Firma: ___________________________'
    ws['A32'].font      = Font(name='Arial', size=11)
    ws['A32'].alignment = LA
    ...
    ws['A34'].value     = ('Generado automáticamente por Cierre de Caja — Grupo Panablock')
    ws['A34'].font      = Font(name='Arial', italic=True, size=9, color='595959')
    ws['A34'].alignment = LA
...
def generate_pdf_report(fecha_str, facturas, recibos, bg, gb,
    ...
    # Row 4:  Desglose sub-hdr         (A-D span)              PALE
    # Row 5-9: formas de pago          (A-C span; D = monto)   VLIGHT
    # Row 10: Total a Depositar        (A-C span; D = efectivo) MGRAY bold
    # Row 11: Ventas al Crédito
    # Row 12: Total Ventas del día     (4 cols, sin recibos)   LGRAY bold
    # ══════════════════════════════════════════════════════════════
    data1 = [
        # 0
        ['VENTAS Y COBROS', '', '', ''],
        ...
        ['Total Ventas del día',
         money_show(round(contado_sub + credito_sub, 2)),
         money_show(round(contado_itb + credito_itb, 2)),
         money_show(round(total_contado + total_credito, 2))],
    ]
    ...
    style1 = TableStyle(
        ...
        + num_right(1, 3, 2, -1)
        # filas resaltadas
        + bold_bg(10, 10, 0, 3, C_MGRAY)
        + bold_bg(12, 12, 0, 3, C_LGRAY)
        # sub-encabezado desglose (fila 4)
        + [('SPAN',      (0, 4), (3, 4)),
           ('BACKGROUND', (0, 4), (3, 4), C_PALE),
           ('TEXTCOLOR',  (0, 4), (3, 4), C_DARK),
           ('FONTNAME',   (0, 4), (3, 4), FONT_B),
           ('LEFTPADDING',(0, 4), (3, 4), 10)]
        # filas de desglose (5-9)
        + [('BACKGROUND', (0, 5), (3, 9), C_VLIGHT)]
        + [('SPAN', (0, r), (2, r)) for r in range(5, 10)]
        + [('LEFTPADDING', (0, 5), (0, 9), 18)]
        # spans: fila 3 y 10 (A-C label, D valor)
        + [('SPAN', (0, 3), (2, 3)),
           ('SPAN', (0, 10), (2, 10))]
    )
    ...
    data2 = [
        ['PAGOS Y GASTOS', ''],
        ['Descripción', 'Total'],
        ['Pago a Proveedores', money_show(total_gast)],
    ]

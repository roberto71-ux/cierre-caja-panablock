...
def parse_recibos(path, log=print):
    ...
    Tipos de recibo reconocidos:
    - A Cuenta (prepago): la columna Tipo dice "A Cuenta"; el monto está en
      la columna Total de esa misma fila (patrón: A Cuenta DD-MM-YYYY monto).
    - Normal (aplicado a facturas): tiene línea "Totales: monto" o montos
      en columna con saldo 0.00.
    """
    paths = path if isinstance(path, list) else [path]
    total = 0.0
    ...
    for p in paths:
        ...
        # Fila tabla: "12-000001323  A Cuenta  13-07-2026  1,498.00  1,498.00"
        acuenta_m = re.search(
        ...
        if acuenta_m:
            ...
            # ── Caso 2: Recibo normal (aplicado a facturas) ──────────────
            tot_m = re.search(r'Totales?:\s*([\d,]+\.\d{2})', text, re.IGNORECASE)
            if tot_m:
                subtotal = _float(tot_m.group(1))
                log(f"  Recibo normal — total: ${subtotal:,.2f}")
            else:
                # Fallback: sumar montos donde el saldo resultante es 0.00
                items = re.findall(r'\b(\d{1,3}(?:,\d{3})*\.\d{2})\s+0\.00', text)
                subtotal = sum(_float(t) for t in items)
                log(f"  Recibo normal (suma individual): ${subtotal:,.2f}")
...
def generate_report(fecha_str, facturas, recibos, bg, gb,
    ...
    row_s3(31, 'Saldo Bancario al Final del Día',
           b='=B28+B29-B30', c='=C28+C29-C30',
           bold=True, bg_color=LGRAY)
    ...
    row_s3(33, 'Saldo Bancario Real Conciliado',
           b='=B31-B32', c='=C31-C32',
    ...
    if ncs:
        ...
        for k, nc in enumerate(ncs):
            r1 = 41 + k * 2   # fila de referencia y montos
            r2 = 42 + k * 2   # fila de cliente y motivo

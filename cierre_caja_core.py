...
def parse_recibos(path, log=print):
    """Retorna total de cobros a clientes (suma de todos los recibos del PDF)."""
    data = _to_bytes(path)
    with pdfplumber.open(io.BytesIO(data)) as pdf:
    ...
    if tot_m:
        total = _float(tot_m.group(1))
        log(f"  Total cobros: ${total:,.2f}")
        return round(total, 2)

    # Fallback: sumar columna de totales individuales
    totals = re.findall(r'\b(\d{1,3}(?:,\d{3})*\.\d{2})\s+0\.00', text)
    total  = sum(_float(t) for t in totals)
    log(f"  Total cobros (suma individual): ${total:,.2f}")
    return round(total, 2)
...
def generate_report(fecha_str, facturas, recibos, bg, gb,
    ...
    row_s3(30, 'Efectivo en tránsito', b='=D10')

    row_s3(31, 'Cheques en Circulación')
    ws['B31'].fill = mk_fill('FFFF00')
    ws['C31'].fill = mk_fill('FFFF00')

    row_s3(32, 'Saldo Bancario al Final del Día',
           b='=B28+B29-B31', c='=C28+C29-C31',
           bold=True, bg_color=LGRAY)
    ws['D31'].border = NO_BORDER
    ...
    ws['A34'].value     = 'Firma: ___________________________'
    ws['A34'].font      = Font(name='Arial', size=11)
    ws['A34'].alignment = LA
    ...
    ws['A36'].value = ('Leyenda:  Celda en amarillo = ingresar manualmente  '
                       '(solo Cheques en Circulación)')
    ws['A36'].font      = Font(name='Arial', italic=True, size=9, color='595959')
    ws['A36'].alignment = LA
    ws['A36'].fill      = mk_fill('FFFF00')
    ...
    if ncs:
        ...
        ws['A38'].value     = 'NOTAS DE CRÉDITO DEL DÍA'
        ws['A38'].font      = Font(name='Arial', bold=True, size=11, color=WHITE)
        ws['A38'].fill      = mk_fill(RED_FILL)
        ws['A38'].alignment = CA

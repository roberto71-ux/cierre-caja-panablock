...
def generate_report(fecha_str, facturas, recibos, bg, gb,
    ...
    row_s3(32, 'Efectivo en tránsito', b='=D10')

    row_s3(33, 'Cheques en Circulación')
    ws['B33'].fill = mk_fill('FFFF00')
    ws['C33'].fill = mk_fill('FFFF00')

    row_s3(34, 'Saldo Bancario Real Conciliado',
           b='=B31+B32-B33', c='=C31+C32-C33',
           bold=True, bg_color=MGRAY)
    ws['D33'].border = NO_BORDER
    ...
    ws['A36'].value     = 'Firma: ___________________________'
    ws['A36'].font      = Font(name='Arial', size=11)
    ws['A36'].alignment = LA
    ...
    ws['A38'].value = ('Leyenda:  Celda en amarillo = ingresar manualmente  '
                       '(solo Cheques en Circulación)')
    ws['A38'].font      = Font(name='Arial', italic=True, size=9, color='595959')
    ws['A38'].alignment = LA
    ws['A38'].fill      = mk_fill('FFFF00')
    ...
    if ncs:
        ...
        ws['A40'].value     = 'NOTAS DE CRÉDITO DEL DÍA'
        ws['A40'].font      = Font(name='Arial', bold=True, size=11, color=WHITE)
        ws['A40'].fill      = mk_fill(RED_FILL)
        ws['A40'].alignment = CA
